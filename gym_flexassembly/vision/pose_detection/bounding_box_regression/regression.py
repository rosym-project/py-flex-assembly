...
class Regressor():
        ...
        if model_type == Model.SVR:
            if not model_params is None:
                self.reg = make_pipeline(StandardScaler(), MultiOutputRegressor(
                    SVR(C=model_params["multioutputregressor__estimator__C"],
                        gamma=model_params["multioutputregressor__estimator__gamma"])))
            else:
                self.reg = make_pipeline(StandardScaler(), MultiOutputRegressor(SVR()))
    ...
    def train_hyperparameters(self, x, y, n_jobs):
        ...
        if self.model_type == Model.SVR:
            parameters = {'multioutputregressor__estimator__C':[0.0001, 0.001, 0.01, 0.1, 1, 10],
                        'multioutputregressor__estimator__gamma':[0.0001, 0.001, 0.01, 0.1, 1, 10]}
...
def main(args):
    ...
    reg = Regressor()
    reg.config(Model.DECISION_TREE_REGRESSOR, None)
