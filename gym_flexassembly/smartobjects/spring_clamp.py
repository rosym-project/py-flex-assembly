...
class SpringClamp:
    ...
    def reset(self):
        if self._use_inertia_from_urdf:
            self._model_id = p.loadURDF(os.path.join(self._urdfRoot_flexassembly+"/smartobjects/W_QS_1", "W_QS_1_test.urdf"), useFixedBase=False, flags = p.URDF_USE_INERTIA_FROM_FILE)
        else:
            self._model_id = p.loadURDF(os.path.join(self._urdfRoot_flexassembly+"/smartobjects/W_QS_1", "W_QS_1_test.urdf"), useFixedBase=False)
